function out = func2(tau,c,kappa)
...
coeff = tau/(1-kappa*tau/h^2);  % Coefficient used by all schemes
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % [K,H] = size(BVW);
sigma = 2*h;
a = 1/sqrt(2*pi*sigma)*exp(-x.^2/(2*sigma^2));
...
% %   if strcmp(method,'FTCS')      %%% FTCS method %%%
    a(1:N) = -c/(2*h)*tau*(a(ip)-a(im)) + kappa/h^2*tau*(a(im)+a(ip)-2*a(1:N)) + a(1:N);
%   elseif strcmp(method,'Lax')  %%% Lax method %%%
...
out = (sum(a)>(1-10*eps)*A0) &&(sum(a)<(1+10*eps)*A0);
%Functie stabiel als out = 1 en instabiel als out = 0
end