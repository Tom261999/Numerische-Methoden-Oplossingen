...
% First and last rows for periodic boundary conditions
 ham(1,N) = 0;   ham(1,1) = -2*coeff; ham(1,2) = coeff;
ham(N,N-1) = coeff; ham(N,N) = -2*coeff; ham(N,1) = 0;

%Maakt matrix Q
Q = 1/2.*(speye(N)+(1i*tau/(2*h_bar)).*ham);
...
k0 = 1*pi/L; %Nieuw golfgetal opgelegd door de potentiaalput
sigma0 = L/10;   % Standard deviation of the wavefunction
Norm_psi = sqrt(2/L);  % Normalization
psi = Norm_psi * sin(n*pi*x'/L);
...
iplot = 1;
figure(2); clf;
axisV = [0 L 0 max(p_plot)]; % Fix axis min and max
