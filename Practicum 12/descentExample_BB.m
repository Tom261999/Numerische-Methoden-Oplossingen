...
[z,zPath] = descent_BB(@myQuadraticFunctionWrapper, xInitial, tol, maxSteps);

% plot path of descent
hold on
plot(xPath(:,1),xPath(:,2),'.-r','MarkerSize',10)
plot(zPath(:,1),zPath(:,2),'.-b','MarkerSize',10)
legend('contourplot','Gradient methode','Barzilai-Borwein methode')
hold off
...
errorz = sqrt(sum((zPath-xMinExact).^2,2));
semilogy(error,'.-'), hold on
semilogy(errorz,'.-'), hold off
legend('error gradient','error Barzilai-Borwein')
